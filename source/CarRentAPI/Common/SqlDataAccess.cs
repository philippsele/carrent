...
namespace CarRentAPI.Common
{
    public class SqlDataAccess
    {
        ...
        public void CreateDBShema()
        {
            ...
            SaveData("Create Table CarClass(ClassId int not null, Class varchar(15) not null, PricePerDay decimal(7,2) not null, primary key ( ClassId))");
            ...
            SaveData("Create Table Car(CarId INT NOT NULL auto_increment, ClassId int not null, TypeId varchar(36) not null, primary key (CarId), foreign key (ClassId) references CarClass(ClassId), foreign key (TypeId) references CarType(TypeId))");
            SaveData("Create Table Contract(ContractId INT NOT NULL auto_increment, CustomerId INT not null, CarId INT not null, PickedUp bit not null, Days int not null, Total decimal(10,2) not null, primary key ( ContractId), foreign key (CustomerId) references Customer(CustomerId), foreign key (CarId) references Car(CarId))");
            ...
            SaveData("Create View ContractV AS Select con.ContractId, cus.CustomerId, cus.FirstName, cus.LastName, cus.Street, cus.City, car.CarId, car.ClassId, car.Class, car.PricePerDay, car.TypeId, car.Type, car.BrandId, car.Brand, con.PickedUp, con.Days, con.Total From Contract con inner join Customer cus on cus.CustomerId = con.CustomerId inner join CarV car on car.CarId = con.CarId");
            SaveData("insert into CarClass (ClassId, Class, PricePerDay) Values (1, 'Basic', 29.90)");
            SaveData("insert into CarClass(ClassId, Class, PricePerDay) Values(2, 'Medium', 59.90)");
            SaveData("insert into CarClass(ClassId, Class, PricePerDay) Values(3, 'Luxury', 99.90)");
